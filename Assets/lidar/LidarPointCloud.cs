...
namespace ar2gh.lidar
{
    ...
    public class LidarPointCloud : MonoBehaviour
    {
        ...
        private unsafe void UpdateCameraImage()
        {
            ...
            {
#if UNITY_EDITOR
                CloudUpdateEvent?.Invoke(GenerateDebugGrid());
#endif
                return;
            }
        ...
        private void ReprojectPointCloud()
        {
            ...
            for (var iY = 0; iY < heightDepth; iY++)
            {
                for (var iX = 0; iX < widthDepth; iX++)
                {
                    ...
                    _cloud[iPoint].SnappedPositionWorld.x = Mathf.RoundToInt(1000 * worldPosition.x);
                    _cloud[iPoint].SnappedPositionWorld.y = Mathf.RoundToInt(1000 * worldPosition.y);
                    _cloud[iPoint].SnappedPositionWorld.z = Mathf.RoundToInt(1000 * worldPosition.z);
                }
            }
        ...
        private int _nextDebugPointIndex;

        private LidarPoint[] GenerateDebugGrid()
        {
            var size = 10000;
            var center = 3 * Random.onUnitSphere;
            LidarPoint Generate(int i)
            {
                return new LidarPoint
                {
                    IsValid = true,
                    Color = Random.ColorHSV((i * 1f / size), (i * 1f / size)),
                    WorldPosition = center + Random.onUnitSphere
                };
            }

            return Enumerable.Range(0, size).Select(Generate).ToArray();
        }
